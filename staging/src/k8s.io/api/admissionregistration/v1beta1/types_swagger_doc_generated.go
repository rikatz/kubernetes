...
var map_ParamRef = map[string]string{
	...
	"namespaceSelector":       "namespaceSelector can be used to match multiple param objects on different namespaces. Supply namespaceSelector: {} to match all resources of the ParamKind on all the namespaces.\n\nIf multiple params are found, they are all evaluated with the policy expressions and the results are ANDed together.\n\n`namespaceSelector` is only valid when `selector` is set, and cannot be used when `namespace` or `name` are also set.",
}
