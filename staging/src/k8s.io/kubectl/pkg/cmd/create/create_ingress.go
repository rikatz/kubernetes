...
func NewCmdCreateIngress(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:                   "ingress NAME --class=ingress-class --rule=host/path=service:port[,tls[=secret]] --rule=host/otherpath*=svc:namedport[,tls[=secret]] --default-backend=default-svc:default-port",
